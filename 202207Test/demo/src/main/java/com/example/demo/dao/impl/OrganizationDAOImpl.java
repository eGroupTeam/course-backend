...
public class OrganizationDAOImpl implements OrganizationDAO {
    ...
    public Organization get(int id) throws Exception {
        Organization organization = new Organization(-1, "", "", "", 0, "", "");
    ...
    public int insert(Organization organization) throws Exception {
        int result = 0;
        String sql = "insert into organization (name, set_time, `desc`, contactnum, mail, address) values(?, ?, ?, ?, ?, ?)";
        try (
                Connection conn = dataSource.getConnection();
                PreparedStatement stmt = conn.prepareStatement(sql);) {
            stmt.setString(1, organization.getName());
            stmt.setString(2, organization.getSet_time());
            stmt.setString(3, organization.getDesc());
            stmt.setInt(4, organization.getContactnum());
            stmt.setString(5, organization.getMail());
    ...
    public int update(Organization organization) throws Exception {
        int result = 0;
        String sql = "update organization set name=?, set_time=?, `desc`=?, contactnum=?, mail=?, address=? where id =?";
        try (
                Connection conn = dataSource.getConnection();
                PreparedStatement stmt = conn.prepareStatement(sql);) {
            stmt.setString(1, organization.getName());
