...
public class EmployeeDAOImpl implements EmployeeDAO {
  ...
  public Employee get(Long id) throws Exception{
    ...
    String sql = "select id, name, department, wage from employees where id = ?";
  ...
  public List<Employee> getNameList(String name) throws Exception{
    ...
    String sql = "select id, name, department, wage from employees where name like ?";
    //當 Connection與PreparedStatement無法取得時，會closeConnection與PreparedStatement
    try (
      Connection conn = dataSource.getConnection();
      PreparedStatement stmt = conn.prepareStatement(sql);){
  ...
  public List<Employee> getList() throws Exception{
    List<Employee> employees = new ArrayList<Employee>();
    String sql = "select id, name, department, wage from employees";
    try(
      Connection conn = dataSource.getConnection();
      PreparedStatement stmt = conn.prepareStatement(sql);
  ...
  public List<Employee> getDepartmentOrderedList() throws Exception{
    List<Employee> employees = new ArrayList<Employee>();
    String sql = "select id, name, department, wage from employees order by department";
    try(
      Connection conn = dataSource.getConnection();
      PreparedStatement stmt = conn.prepareStatement(sql);
      ResultSet rs = stmt.executeQuery();) {
  ...
  public List<Employee> getWageBetweenList(int lowBound, int upperBound) throws Exception{
    List<Employee> employees = new ArrayList<Employee>();
    String sql = "select id, name, department, wage from employees where wage between ? and ?";
    try(
      Connection conn = dataSource.getConnection();
      PreparedStatement stmt = conn.prepareStatement(sql);) {
  ...
  public List<DepartmentCount> getGroupByDepartmentList() throws Exception{
    List<DepartmentCount> dcounts = new ArrayList<DepartmentCount>();
    String sql = "select department, COUNT(department) As count from employees group by department";
    try(
      Connection conn = dataSource.getConnection();
      PreparedStatement stmt = conn.prepareStatement(sql);
      ResultSet rs = stmt.executeQuery();) {     
      while (rs.next()){
        ...
        dcounts.add(new DepartmentCount(rs.getString("department"), rs.getInt("count")));
      }  
    } catch(Exception e) {
      throw e;
    }
  ...
  public List<String> getGroupByDepartmentHavingList() throws Exception{
    List<String> employees = new ArrayList<String>();
    String sql = "select department from employees group by department having COUNT(department)=1";
    try(
      Connection conn = dataSource.getConnection();
      PreparedStatement stmt = conn.prepareStatement(sql);) {
      try(ResultSet rs = stmt.executeQuery();){
  ...
  public int insert(Employee employee) throws Exception{
    int result = 0;
    String sql = "insert into employees (name, depatrment, wage) values(?, ?, ?)";
    try( 
      Connection conn = dataSource.getConnection();
      PreparedStatement stmt = conn.prepareStatement(sql);) {    
  ...
  public int update(Employee employee) throws Exception{
    int result = 0;
    String sql = "update employees set name=?, department=?, wage=? where id =?";
    try(
      Connection conn = dataSource.getConnection();
      PreparedStatement stmt = conn.prepareStatement(sql);) {
      stmt.setString(1, employee.getName());
